...
public:
    ...
    virtual void visitUnknown() { result = "unknown"; }
...
const char * identifyExprClass(nix::Expr * expr)
{
...
public:
    ExprDepthFirstSearch(ExprVisitor * innerVisitor)
    {
        this->v = innerVisitor;
    }

    using ExprVisitor::visit;
    virtual void visitNull() { v->visitNull(); }
    virtual void visitUnknown() { v->visitUnknown(); }
...
int main(int argc, char ** argv)
{
    return nix::handleExceptions(argv[0], [&]() {
        ...
        std::cout << "mainExpr: " << *mainExpr << std::endl << std::endl;

        nix::ExprLambda * mainLambda = dynamic_cast<nix::ExprLambda *>(mainExpr);
        if (mainLambda == nullptr)
        {
            throw std::runtime_error("Expression is not a lambda.");
        }

        nix::ExprApp * bodyApp = dynamic_cast<nix::ExprApp *>(mainLambda->body);
        nix::ExprSelect * select = dynamic_cast<nix::ExprSelect *>(bodyApp->e1);
        nix::ExprVar * var = dynamic_cast<nix::ExprVar *>(select->e);

        std::cout << var->name << std::endl << std::endl;
        //std::cout << *select->attrPath << std::endl;

        std::cout << *select << std::endl;
        std::cout << var->pos << std::endl << std::endl;
